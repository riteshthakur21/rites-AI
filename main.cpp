...
public:
    ...
    // ==================================================
    // 💾 NAYA UPGRADE: DATA PERSISTENCE (DISK SAVE & LOAD)
    // ==================================================
    ...
    bool save_to_disk(const std::string& filename) {
        ...
        for (auto& [id, v] : store) {
            ...
            size_t mlen = v.metadata.size();
            out.write(reinterpret_cast<const char*>(&mlen), sizeof(mlen));
            out.write(v.metadata.c_str(), mlen);

            size_t clen = v.category.size();
            out.write(reinterpret_cast<const char*>(&clen), sizeof(clen));
            out.write(v.category.c_str(), clen);

            size_t elen = v.emb.size();
            out.write(reinterpret_cast<const char*>(&elen), sizeof(elen));
            out.write(reinterpret_cast<const char*>(v.emb.data()), elen * sizeof(float));
        }
        return out.good();
    }

    bool load_from_disk(const std::string& filename, DistFn dist) {
        std::lock_guard<std::mutex> lk(mu);
        ...
        store.clear();
        std::vector<VectorItem> itemsForTree;
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            v.metadata.resize(mlen);
            in.read(&v.metadata[0], mlen);

            size_t clen;
            in.read(reinterpret_cast<char*>(&clen), sizeof(clen));
            v.category.resize(clen);
            in.read(&v.category[0], clen);

            size_t elen;
            in.read(reinterpret_cast<char*>(&elen), sizeof(elen));
            v.emb.resize(elen);
            in.read(reinterpret_cast<char*>(v.emb.data()), elen * sizeof(float));

            store[v.id] = v;
            itemsForTree.push_back(v);

            // Re-insert into HNSW and BruteForce (KDTree rebuilds below)
            bf.insert(v);
            hnsw.insert(v, dist);
        }

        kdt.rebuild(itemsForTree); // KDTree ek saath rebuild hota hai
        return true;
    }
};
// class VectorDB {
...
public:
    ...
    bool save_to_disk(const std::string& filename) {
        ...
        for (auto& [id, item] : store) {
            ...
            size_t tlen = item.title.size();
            out.write(reinterpret_cast<const char*>(&tlen), sizeof(tlen));
            out.write(item.title.c_str(), tlen);

            size_t xlen = item.text.size();
            out.write(reinterpret_cast<const char*>(&xlen), sizeof(xlen));
            out.write(item.text.c_str(), xlen);

            size_t elen = item.emb.size();
            out.write(reinterpret_cast<const char*>(&elen), sizeof(elen));
            out.write(reinterpret_cast<const char*>(item.emb.data()), elen * sizeof(float));
        }
        return out.good();
    }
    ...
    bool load_from_disk(const std::string& filename) {
        ...
        for (size_t i = 0; i < count; ++i) {
            DocItem item;
            in.read(reinterpret_cast<char*>(&item.id), sizeof(item.id));

            size_t tlen;
            in.read(reinterpret_cast<char*>(&tlen), sizeof(tlen));
            item.title.resize(tlen);
            in.read(&item.title[0], tlen);

            size_t xlen;
            in.read(reinterpret_cast<char*>(&xlen), sizeof(xlen));
            item.text.resize(xlen);
            in.read(&item.text[0], xlen);

            size_t elen;
            in.read(reinterpret_cast<char*>(&elen), sizeof(elen));
            item.emb.resize(elen);
            in.read(reinterpret_cast<char*>(item.emb.data()), elen * sizeof(float));

            store[item.id] = item;
            
            // Wapas HNSW graph aur BruteForce mein insert karna
...
int main() {
    ...
    if (docDB.load_from_disk("my_docs.bin")) {
        std::cout << "Loaded " << docDB.size() << " saved documents from disk!\n";
    }
    ...
    svr.Post("/doc/insert", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        docDB.save_to_disk("my_docs.bin");
    ...
    svr.Delete(R"(/doc/delete/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (ok) docDB.save_to_disk("my_docs.bin");
